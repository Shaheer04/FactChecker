(function () { function r(e, n, t) { function o(i, f) { if (!n[i]) { if (!e[i]) { var c = "function" == typeof require && require; if (!f && c) return c(i, !0); if (u) return u(i, !0); var a = new Error("Cannot find module '" + i + "'"); throw a.code = "MODULE_NOT_FOUND", a } var p = n[i] = { exports: {} }; e[i][0].call(p.exports, function (r) { var n = e[i][1][r]; return o(n || r) }, p, p.exports, r, e, n, t) } return n[i].exports } for (var u = "function" == typeof require && require, i = 0; i < t.length; i++)o(t[i]); return o } return r })()({
    1: [function (require, module, exports) {
        // This file contains the background script for the extension. It sets up event listeners for context menu interactions and handles the logic for invoking the Gemini API when the context menu option is selected.

        chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
            if (request.action === "backgroundHandshake") {
                console.log("Background script loaded");
                sendResponse({ status: "Background script loaded" });
            }
        })
        async function generateContent(text) {
            ...
            const requestPayload = {
                ...
                "justification": string, // justification for the statement being true or false
        ...
        chrome.contextMenus.onClicked.addListener((info) => {
            if (info.menuItemId === "factCheck") {
                ...
                chrome.action.openPopup().then(
                    async () => await chrome.runtime.sendMessage({ action: "factCheckLoading", loading: true })
                );
                chrome.storage.local.set({ loading: true });
                generateContent(selectedText).then((result) => {
                    console.log("generateContent.then", result);
                    ...
                    chrome.runtime.sendMessage({ action: "factCheckLoaded", result }).then(
                        () => {
                            console.log("factCheckLoaded sent");
                        }
                    ).catch((error) => {
                        console.error("Error sending factCheckLoaded message:", error);
                    }
                    );
                })
            }
